...
class Donate_Module extends Module {
    ...
    public function __construct($language, $donate_language, $pages, $user, $queries, $navigation, $cache){
        ...
        // Register event
        HookHandler::registerEvent('newDonation', $this->_donate_language->get('admin', 'new_donation'));
    ...
    public function onPageLoad($user, $pages, $cache, $smarty, $navs, $widgets, $template){
        ...
        if(defined('BACK_END')){
			...
			PermissionHandler::registerPermissions($this->_donate_language->get('general', 'donate'), array(
        ...
        if(isset($_GET['route']) && $user->isLoggedIn() && $user->hasPermission('admincp.update')){
            ...
            if($page['module'] == 'Donate'){
                ...
                if(!isset($update_check->error) && !isset($update_check->no_update) && isset($update_check->new_version)){  
                    $smarty->assign(array(
                        'NEW_UPDATE' => str_replace('{x}', $this->getName(), (isset($update_check->urgent) && $update_check->urgent == 'true') ? $this->_donate_language->get('admin', 'new_urgent_update_available_x') : $this->_donate_language->get('admin', 'new_update_available_x')),
                        'NEW_UPDATE_URGENT' => (isset($update_check->urgent) && $update_check->urgent == 'true'),
                        'CURRENT_VERSION' => str_replace('{x}', $this->getVersion(), $this->_donate_language->get('admin', 'current_version_x')),
                        'NEW_VERSION' => str_replace('{x}', Output::getClean($update_check->new_version), $this->_donate_language->get('admin', 'new_version_x')),
	...
	private function initialise(){
		...
		if(!$queries->tableExists('donate_payments')) {
			try {
				$queries->createTable('donate_payments', ' `id` int(11) NOT NULL AUTO_INCREMENT, `user_id` int(11) NOT NULL DEFAULT \'0\', `payment_method` int(11) NOT NULL, `transaction_id` varchar(32) DEFAULT NULL, `payer_email` varchar(64) DEFAULT NULL, `amount` varchar(11) DEFAULT NULL, `currency` varchar(11) DEFAULT NULL, `fee` varchar(11) DEFAULT NULL, `status_id` int(11) NOT NULL DEFAULT \'0\', `created` int(11) NOT NULL, `last_updated` int(11) NOT NULL, PRIMARY KEY (`id`)', "ENGINE=$engine DEFAULT CHARSET=$charset");
			} catch(Exception $e){
				// Error
			}
		}
        
        if(!$queries->tableExists('donate_settings')) {
			try {
				$queries->createTable('donate_settings', ' `id` int(11) NOT NULL AUTO_INCREMENT, `name` varchar(64) NOT NULL, `value` text, PRIMARY KEY (`id`)', "ENGINE=$engine DEFAULT CHARSET=$charset");
