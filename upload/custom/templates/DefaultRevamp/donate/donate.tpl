      ...
      {foreach from=$LATEST_DONATIONS_LIST item=donator}
        <div class="ui relaxed list">
          <div class="item">
            ...
            <div class="content">
              <a class="header" href="{$donator.profile_url}" data-toggle="popup" data-position="top left">{$donator.username}</a>
              <p>${$donator.amount} {$donator.currency}</br><span data-toggle="tooltip" data-content="{$donator.date}">{$donator.date_rough}</span></p>
            </div>
          </div>
        </div>
      {/foreach}
